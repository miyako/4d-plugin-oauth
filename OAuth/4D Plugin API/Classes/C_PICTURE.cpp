...
PA_ErrorCode CPicture::getPictureDataTypeAtIndex(uint32_t index, CUTF8String *pictureData)
{
	PA_ErrorCode err = eER_NoErr;
	PA_Unistring type;
	
	type = PA_GetPictureData(this->_CPicture, index, 0);
	err = PA_GetLastError();
	
	if(err == eER_NoErr){	
		
		uint32_t size = ((uint32_t)type.fLength * 4) + sizeof(uint8_t);		
		std::vector<uint8_t> buf(size);		
		
		uint32_t len = PA_ConvertCharsetToCharset(
												  (char *)type.fString,
												  (uint32_t)type.fLength * sizeof(PA_Unichar),
												  eVTC_UTF_16,
												  (char *)&buf[0],
												  size,
												  eVTC_UTF_8
		...
		*pictureData = CUTF8String((const uint8_t *)&buf[0], len);
...
PA_Picture CPicture::copyPictureAtIndex(uint32_t index)
{
	PA_Picture picture = NULL;
	PA_Handle h = PA_NewHandle(0);
	PA_GetPictureData(this->_CPicture, index, h);
	
	if(PA_GetLastError() == eER_NoErr)
	{
		picture = PA_CreatePicture(PA_LockHandle(h), PA_GetHandleSize(h));
		PA_UnlockHandle(h);
	}
	PA_DisposeHandle(h);
...
PA_ErrorCode C_PICTURE::getPictureDataTypeAtIndex(uint32_t index, CUTF8String *pictureData)
{
	return this->_CPicture->getPictureDataTypeAtIndex(index, pictureData);	
}

PA_Picture C_PICTURE::copyPictureAtIndex(uint32_t index)
{
	return this->_CPicture->copyPictureAtIndex(index);	
}
